...
class ScaleImpactTypesService extends \MonarcMigrationTool\Service\Import\AbstractService{
    ...
    public function import(&$corresp = []){
        if(!empty($corresp['scales'])){
            ...
            $table = $this->serviceLocator->get('\MonarcCore\Model\Table\ScaleImpactTypeTable');
            $scaleService = $this->serviceLocator->get('MonarcCore\Service\ScaleImpactTypeService');
            $c = $table->getClass();
            ...
            foreach($res as $r){
                if(isset($corresp['scales'][$r['scale_id']])){
                    $scaleT = new $c();
                    $scaleT->setDbAdapter($this->serviceLocator->get($this->dbUsed));
